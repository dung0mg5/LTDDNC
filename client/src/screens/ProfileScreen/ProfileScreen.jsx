...
function Profile({route, navigation}) {
  ...
  async function doRequestConnection() {
    const {data, errorMessage} = await requestConnection({
      userId: profile.user.authenticatedUser._id,
    });
  ...
  async function checkConnected() {
    const {data} = await checkUserIsConnected({
      userId: profile.user.authenticatedUser._id,
    });
        ...
        <View>
          <View
            ...
            <View
              ...
              {!isConnected && (
                <Button
                  onHandlePress={user ? doRequestConnection : () => {}}
                  backgroundColor="#2D64BC"
                  colorText="#fff"
                  width={170}
                  textSize={18}
                  padding={6}>
                  {user ? 'Connect' : 'Open to'}
                </Button>
              )}
              <Button
                borderWidth={1}
                borderColor="#2D64BC"
                backgroundColor={isConnected ? '#2D64BC' : '#fff'}
                colorText={isConnected ? '#fff' : '#2D64BC'}
                width={isConnected ? 340 : 170}
                textSize={18}
                padding={6}>
                {user ? 'Message' : 'Add section'}
              </Button>
              <TouchableOpacity>
                <View
                  style={{
          ...
          <TouchableOpacity
            onPress={() => navigation.jumpTo('edit')}
            style={{
              position: 'absolute',
              right: 15,
              top: 15,
